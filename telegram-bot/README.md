...
- Опционально: отдельный **MTProto-прокси (mtg)** на каждого клиента (`MTPROXY_ENABLED=true`, см. `.env.example` и `../mtproxy/README.md`)
...
- `/list_clients` — Список клиентов (страницы из БД, кнопки «Поиск» по подстроке ID/имени и «Сброс» фильтра)
